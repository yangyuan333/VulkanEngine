...
private:
	...
	void cleanup()
	{
		...
		vkDestroyDevice(device, nullptr); // 物理设备不需要手动清除
		if (enableValidationLayers) DestroyDebugUtilsMessengerEXT(instance, callback, nullptr);
...
private:
	...
	void createLogicalDevice()
	{
		...
		// queueCreateInfo.pQueuePriorities = &queuePriority; // 这个有什么用呢？
		...
		// 极大概率是相同的...目前实现就是相同的...上面加了两个 VkDeviceQueueCreateInfo 还有什么意义吗？
		// 意思难道是如果 VkDeviceQueueCreateInfo 的队列族给的一样，就被合并了？看queuecount的最大值？
	...
	void createSwapChain()
	{
		...
		createInfo.imageExtent = extent; // 如果涉及到动态分辨率渲染怎么办呢？
	...
	void createGraphicsPipeline()
	{
		...
		// 不把这两 bake 到管线中，可以动态切换；否则一旦改变，管线就得重新创建；
		// blend constants、line width 也可以动态切换---VkDynamicState里面的都可以；
		// 即使后续配置了相关的属性，也都会被 ignored；
		...
		// 如果前面 dynamic 设置了 viewport 和 scissor，此处只需要设置 viewport scissor 数量即可，大小设置无效
		// 实际的大小需要后续设置
		VkViewport viewport = {};
		viewport.x = 0.0f;
		viewport.y = 0.0f;
	...
	void createDescriptorSetLayout()
	{
		...
		uboLayoutBinding.descriptorCount = 1; // UBO Array
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
	{
		...
		// implicitly reset the commandbuffer
	...
	void createImage(
		...
		VkImage& image, VkDeviceMemory& imageMemory, uint32_t mipLevels = 1, VkSampleCountFlagBits numSamples = VkSampleCountFlagBits::VK_SAMPLE_COUNT_1_BIT)
	{
	...
	VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR> availablePresentModes)
	{
		...
		return VK_PRESENT_MODE_FIFO_KHR; // 一定能保证是有用的
	}
...
private:
	...
	VkSurfaceKHR surface; // Vulkan对窗口的抽象
	...
	const std::string MODEL_PATH = "D:/GraphicsLearning/Code/littleVulkanEngine/build/models/viking_room.obj";
	const std::string TEXTURE_PATH = "D:/GraphicsLearning/Code/littleVulkanEngine/build/textures/viking_room.png";
